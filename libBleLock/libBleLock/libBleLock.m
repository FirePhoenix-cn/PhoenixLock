...
#define DEFAULT_CONNECT_TIMEOUT      5      /* second */
...
@property (strong, nonnull) NSData *remoteMacAddr;
...
-(void) inquiryTimeoutTimer:(NSTimer *)timer
{
    ...
    self->_inquiryTimer = nil;
    self->_isInquiryingPeripherals = FALSE;

    [self writePeripheralsListToFile];
    
    if (self->_isApplicationRemandInquiry) {
        self->_isApplicationRemandInquiry = FALSE;
        if (self->_delegate) {
            [self->_delegate didDiscoverComplete];
        }
    }
    else
    {
        PeripheralDeviceManager *peripheralDevice = [self getPeripheralDeviceFromPeripheralListByMacAddr:self->_remoteMacAddr];
...
#endif
#endif
            [self->_delegate didConnectConfirm:self->_remoteMacAddr status:FALSE];
            self->_remoteMacAddr = nil;
        }
        else
        {
#ifdef NSLOG_DEBUG
            ...
            [self->_centralManager connectPeripheral:peripheralDevice.peerPeripheral options:nil];
...
-(void) connectTimeoutTimer:(NSTimer *)timer
{
    ...
    [self->_centralManager cancelPeripheralConnection:peripheralDevice.peerPeripheral];
    
    if (peripheralDevice.isConnecting && self->_delegate != nil)
    {
        [self->_delegate didConnectConfirm:peripheralDevice.macAddr status:FALSE];
        self->_remoteMacAddr = nil;
    }
...
-(id) initWithDelegate:(id<libBleLockDelegate>)delegate
{
    ...
    self->_centralManager = [[CBCentralManager alloc] initWithDelegate:self queue:nil options:@{CBCentralManagerOptionShowPowerAlertKey:@"0"}];
    self->_isInquiryingPeripherals = FALSE;
    self->_inquiryTimer = nil;
    self->_remoteMacAddr = nil;
    self->_peripheralsList = [[NSMutableArray alloc] initWithCapacity:1];
...
-(Boolean) bleInquiry:(NSTimeInterval)timeoutSeconds
{
    ...
    self->_isApplicationRemandInquiry = TRUE;
    
    if (self->_isInquiryingPeripherals)
    {
        return TRUE;
    }
    self->_inquiryTimer = [NSTimer scheduledTimerWithTimeInterval:timeoutSeconds target:self selector:@selector(inquiryTimeoutTimer:) userInfo:nil repeats:NO];
    
    NSArray *uuidArray = [NSArray arrayWithObjects:[CBUUID UUIDWithString:UUID_SERVICE_FOR_DATA], nil];
    NSDictionary *options = [NSDictionary dictionaryWithObject:[NSNumber numberWithBool:NO] forKey:CBCentralManagerScanOptionAllowDuplicatesKey];
    [self->_centralManager scanForPeripheralsWithServices:uuidArray options:options];
    self->_isInquiryingPeripherals = TRUE;
...
{
    if (self->_isApplicationRemandInquiry && self->_isInquiryingPeripherals) {
        [self->_inquiryTimer fire];
 ...
 *          连接指定Mac地址的Ble设备
 */
-(Boolean) bleConnectRequest:(NSData *)macAddr forbattery:(BOOL)isbattery
{
#ifdef NSLOG_DEBUG
    NSLog(@"bleConnectRequest:%@, %d", self->_remoteMacAddr, self->_isInquiryingPeripherals);
#ifdef PRINT_LOG
    [printLog printLogToFile:@"bleConnectRequest\n"];
#endif
#endif
    if (self->_centralManager.state != CBCentralManagerStatePoweredOn || self->_remoteMacAddr != nil)
    {
        return FALSE;
    }
    ...
    self->_remoteMacAddr = macAddr;
    
    PeripheralDeviceManager *peripheralDevice = [self getPeripheralDeviceFromPeripheralListByMacAddr:self->_remoteMacAddr];
    if (peripheralDevice == nil)
    {
        self->_inquiryTimer = [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(inquiryTimeoutTimer:) userInfo:nil repeats:NO];
        NSArray *uuidArray = [NSArray arrayWithObjects:[CBUUID UUIDWithString:UUID_SERVICE_FOR_DATA], nil];
        NSDictionary *options = [NSDictionary dictionaryWithObject:[NSNumber numberWithBool:NO] forKey:CBCentralManagerScanOptionAllowDuplicatesKey];
        [self->_centralManager scanForPeripheralsWithServices:uuidArray options:options];
        self->_isInquiryingPeripherals = TRUE;
...
-(void) centralManagerDidUpdateState:(CBCentralManager *)central
{
    switch (central.state) {
        ...
        case CBCentralManagerStatePoweredOff:
        {
            ...
            for(NSUInteger i = 0; i < self->_peripheralsList.count; i++)
            {
                peripheralDevice = [self->_peripheralsList objectAtIndex:i];
                ...
                if (self->_delegate != nil) {
                    if(peripheralDevice.isConnected)
                    {
                        [self->_delegate didDisconnectIndication:peripheralDevice.macAddr];
                    }
    ...
    if (self->_isApplicationRemandInquiry) {
        ...
        if (nsdataManufacturerData.length == 6) {
            ...
            else
            {
                ...
                [self->_peripheralsList addObject:peripheralDevice];
            }
            ...
            if (self->_delegate) {
                [self->_delegate didDiscoverResult:peripheralDevice.macAddr deviceName:deviceName rssi:RSSI];
            }
        }
    }
    else if ([nsdataManufacturerData isEqualToData:self->_remoteMacAddr]) {
        if (peripheralDevice) {
            peripheralDevice.peerPeripheral = peripheral;
        }
        else
        ...
        peripheralDevice.advertisementData = advertisementData;
        peripheralDevice.RSSI = RSSI;
        peripheralDevice.macAddr = [[NSData alloc] initWithData:self->_remoteMacAddr];
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    if (! [peripheralDevice discoverPeripheralDeviceServices:self->_isbattery])
    {
#ifdef NSLOG_DEBUG
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    PeripheralDeviceManager *peripheralDevice = [self getPeripheralDeviceFromPeripheralListByCBPeripheral:peripheral];
#ifdef NSLOG_DEBUG
    NSLog(@"didDisconnectPeripheral:%@, %@", peripheralDevice, self->_delegate);
#ifdef PRINT_LOG
...
-(void)peripheralDeviceManager:(PeripheralDeviceManager *)peripheralDevice GetBattery:(NSData *)battery
{
    ...
    [self->_delegate didGetBattery:batnum forMac:peripheralDevice.macAddr];
}
...
-(void) peripheralDeviceManager:(PeripheralDeviceManager *)peripheralDevice didServiceDiscoverResult:(Boolean)success
{
    ...
    if (!success)
    {
        ...
        else if (peripheralDevice.isConnecting && self->_delegate != nil)
        {
            [self->_delegate didConnectConfirm:peripheralDevice.macAddr status:FALSE];
            self->_remoteMacAddr = nil;
        }
    ...
    else if(peripheralDevice.isConnecting)
    {
        ...
        if (self->_delegate != nil)
            [self->_delegate didConnectConfirm:peripheralDevice.macAddr status:TRUE];
    }
}
...
{
    if (self->_delegate != nil)
        [self->_delegate didDataSendResponse:peripheralDevice.macAddr cmd_type:cmd_type result:result param_data:param_data];
    else
    {
        [peripheralDevice dataSendToPeripheraDevice:libBleCmdDisconnect param_data:nil];
        [self->_centralManager cancelPeripheralConnection:peripheralDevice.peerPeripheral];
...
{
    if (self->_delegate != nil)
        [self->_delegate didOpenLockLogDataInd:peripheralDevice.macAddr record_count:record_count log_data:log_data];
    else
    {
        [peripheralDevice dataSendToPeripheraDevice:libBleCmdDisconnect param_data:nil];
        [self->_centralManager cancelPeripheralConnection:peripheralDevice.peerPeripheral];
...
-(void) readPeripheralsListFromFile
{
    ...
    {
        peripheralsArray = [self->_centralManager retrievePeripheralsWithIdentifiers:uuidMutableArray];
